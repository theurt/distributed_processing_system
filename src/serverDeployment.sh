...
then
	...
	if [ "${isIp}" == "0" ] && [ "${islocal}" == "0" ]
	then
		echo "format : ./serverDeployment.sh [127.0.0.1:8080]"
		exit -1
	else
		adresse=$1
	fi
...
echo "java linda.server.LindaServerImpl ${adresse}"
gnome-terminal --title="Server ${adresse}" -- bash -c "java linda.server.LindaServerImpl ${adresse} ; bash"
